...
class Preprocessing():
    """Converts the PDF into TIFF, crops the image into 8 rows,
    segments them into digits and saves the digits.
    """
    ...
    def clear_images(cls):
        ...
        for file in glob.glob("./data/roi/*.png"):
            path = pathlib.Path(file)
            path.unlink()
        for file in glob.glob("./data/raw/*.pdf"):
            path = pathlib.Path(file)
            path.unlink()
    ...
    def crop(self, image):
        ...
        if image.shape > (500, 500):
            ...
            main_region = self.row_detection(image=main_region)
