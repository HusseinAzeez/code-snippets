...
def predict(path):
    ...
    length_model = load_model('./lib/models/length3.h5',
                              custom_objects={'PoolHelper': PoolHelper(), 'LRN2D': LRN2D()})
    single_model = load_model('./lib/models/single_mix3.h5',
                              custom_objects={'PoolHelper': PoolHelper(), 'LRN2D': LRN2D()})
    double_model = load_model(
        './lib/models/double2.h5', custom_objects={'PoolHelper': PoolHelper(), 'LRN2D': LRN2D()})
